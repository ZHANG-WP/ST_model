...
def load_trained_model_for_inference(model_training_output_dir, use_folds, checkpoint_name):
    ...
    if isinstance(use_folds, str):
        use_folds = [use_folds] # In our case : 4
    ...
    for i, f in enumerate(use_folds):
        ...
        if i == 0:
            trainer_name = checkpoint['trainer_name'] # In our case : nnUNetTrainer
            configuration_name = checkpoint['init_args']['configuration'] # In our case : 3D_fullres
            inference_allowed_mirroring_axes = checkpoint['inference_allowed_mirroring_axes'] if \
                'inference_allowed_mirroring_axes' in checkpoint.keys() else None # In our case : (0,1,2)
...
def predict_from_raw_data(list_of_lists_or_source_folder: Union[str, List[List[str]]],
                          ...
                          verbose: bool = True,
                          save_probabilities: bool = True, # Default = False
                          overwrite: bool = True,
    ...
    """
    各參數解釋：
    list_of_lists_or_source_folder : 帶預測資料夾路徑(ImageTs)
    output_folder_or_list_of_truncated_output_files : 預設結果輸出資料夾
    model_training_output_dir : 訓練模型所產生的相關文件的存儲位置
    use_folds : 用於交叉驗證的折數(folds)
    tile_step_size : 圖像切片過程中使用的步長
    use_gaussian : 是否在預測過程中使用高斯過濾器
    use_mirroring : 是否在預測過程中使用鏡像
    perform_everything_on_gpu : 是否將所有計算操作執行在GPU上
    verbose : 是否顯示詳細的執行信息
    save_probabilities : 是否保存預測的機率值 Default = False
    overwrite : 是否覆蓋現有的輸出文
    checkpoint_name : 訓練模型時保存的檢查點文件名
    num_processes_preprocessing : 用於預處理的進程數
    num_processes_segmentation_export : 用於分割導出的進程數
    folder_with_segs_from_prev_stage : 先前階段分割結果的文件夾路徑
    num_parts : 總部分數
    part_id : 當前部分數id
    device : 模型運行的硬體裝置
    """
    ...
    if device.type != 'cuda':
        perform_everything_on_gpu = False # 如果不是 CUDA 設備，禁用 GPU 運算

    ########################
    # let's store the input arguments so that its clear what was used to generate the prediction
    # 儲存輸入參數，以便清楚知道生成預測的輸入條件
    if isinstance(output_folder_or_list_of_truncated_output_files, str): # isinstance用以確認變數格式
    ...
    if output_folder is not None:
        my_init_kwargs = {}
        """
        動態地獲取 predict_from_raw_data 函式的參數名稱，然後使用 locals() 函式動態地從當前函式的局部命名空間中獲取每個參數的值
        ，並將其保存到一個字典 my_init_kwargs 中 (只會紀錄呼叫函式時的設定參數，默認參數並不會紀錄)。
        """
        ...
        # 將訓練資料夾中的data.json跟plan.json複製到output_folder中，以利若需要使用後處理
    ...
    # load all the stuff we need from the model_training_output_dir, 讀取所有在model中所需要的資訊
    ...
    # check if we need a prediction from the previous stage, 檢查是否需要從先前階段獲取預測
    ...
    # sort out input and output filenames, 整理輸入和輸出文件名
...
def predict_from_data_iterator(data_iterator,
                               ...
                               num_processes_segmentation_export: int = default_num_processes,
                               device: torch.device = torch.device('cuda')):
    # 每個由 data_iterator 返回的元素必須是一個帶有 'data'、'ofile' 和 'data_properites' 鍵的字典！
    """
    each element returned by data_iterator must be a dict with 'data', 'ofile' and 'data_properites' keys!
    """
    # 使用 "spawn" 上下文創建一個多進程池，用於將預測的結果導出到文件
    with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
        network = network.to(device) # 移動模型到指定的設備（GPU 或 CPU）

        r = []
        # 用 torch.no_grad() 上下文，確保在推論過程中不計算梯度
        with torch.no_grad():

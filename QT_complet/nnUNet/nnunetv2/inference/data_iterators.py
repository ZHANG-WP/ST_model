...
def get_data_iterator_from_lists_of_filenames(input_list_of_lists: List[List[str]],
                                              ...
                                              pin_memory: bool,
                                              verbose: bool = False):
    # 創建一個數據預處理器（preprocessor），這個 preprocessor 是根據 configuration_manager 中的設定而實例化的
    preprocessor = configuration_manager.preprocessor_class(verbose=verbose)
    # hijack batchgenerators, yo
    # we use the multiprocessing of the batchgenerators dataloader to handle all the background worker stuff. This
    # way we don't have to reinvent the wheel here.

    # 使用 BatchGenerators 的多進程數據加載器（dataloader）處理背景工作
    # 我們使用 batchgenerators dataloader 的多進程功能來處理所有背景工作者的事務。這樣我們就不必在這裡重新發明輪子。
    num_processes = max(1, min(num_processes, len(input_list_of_lists)))

    # 創建 PreprocessAdapter 對象，用於適應 preprocessor、input_list_of_lists、seg_from_prev_stage_files、output_filenames_truncated 等信息
    ...
    # 根據指定的進程數量創建 MultiThreadedAugmenter 或 SingleThreadedAugmenter
    if num_processes == 0:

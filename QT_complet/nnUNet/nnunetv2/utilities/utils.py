...
def get_identifiers_from_splitted_dataset_folder(folder: str, file_ending: str):
    files = subfiles(folder, suffix=file_ending, join=False) # 獲取指定資料夾中以指定檔案結尾結束的所有檔案清單
    # all files must be .nii.gz and have 4 digit channel index
    crop = len(file_ending) + 5 # 計算要截斷的字符數，包括檔案結尾的長度和 5 個額外的字符
    files = [i[:-crop] for i in files] # 去掉最後 crop 個字符
    # only unique image ids
    files = np.unique(files) # 確保返回的檔案識別符是唯一的
    return files

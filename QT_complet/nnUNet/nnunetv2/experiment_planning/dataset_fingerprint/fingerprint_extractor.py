...
class DatasetFingerprintExtractor(object):
    ...
    def run(self, overwrite_existing: bool = False) -> dict:
        ...
        preprocessed_output_folder = join(nnUNet_preprocessed, self.dataset_name) # 將 nnUNet_preprocessed 目錄與資料集名稱結合，建立了輸出資料夾的路徑
        maybe_mkdir_p(preprocessed_output_folder)
        properties_file = join(preprocessed_output_folder, 'dataset_fingerprint.json') # 定義 'dataset_fingerprint.json' 屬性檔案的路徑

        if not isfile(properties_file) or overwrite_existing:
            file_ending = self.dataset_json['file_ending'] # 取得dataset_json中的file_ending資料(我們的為.nii.gz)
            training_identifiers = get_identifiers_from_splitted_dataset_folder(join(self.input_folder, 'imagesTr'),
                                                                                file_ending) # 取得\nnUNet_raw\imagesTr資料夾中所有檔案識別符列表
